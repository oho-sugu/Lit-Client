...
//            Debug.Log("*************************** JobStatusAsync ***************************");
    ...
    public class JobCaptureAsync : JobAsync
    {
        ...
        public override async Task RunJobAsync()
        {
            ...
            var byteStream = new ProgressMemoryStream(body, this.Progress);
            request.Content = new StreamContent(byteStream);

            using (MemoryStream stream = new MemoryStream())
            {
                using (var response = await ImmersalSDK.client.DownloadAsync(request, stream, null, CancellationToken.None))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        string responseBody = Encoding.ASCII.GetString(stream.GetBuffer());
    ...
    public class JobGeoPoseAsync : JobAsync
    {
        ...
        public override async Task RunJobAsync()
        {
            ...
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, string.Format(Endpoint.URL_FORMAT, this.server, Endpoint.GEOPOSE));
            var byteStream = new ProgressMemoryStream(body, this.Progress);
            request.Content = new StreamContent(byteStream);
            ...
            using (MemoryStream stream = new MemoryStream())
            {
                using (var response = await ImmersalSDK.client.DownloadAsync(request, stream, null, CancellationToken.None))
                {
                    ...
                    {
                        string responseBody = Encoding.ASCII.GetString(stream.GetBuffer());
                        SDKGeoPoseResult result = JsonUtility.FromJson<SDKGeoPoseResult>(responseBody);
                        this.OnResult?.Invoke(result);
                    }
                    else
                    {
